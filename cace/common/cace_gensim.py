...
def construct_dut_from_path(pname, pathname, pinlist):
        ...
        if lmatch:
            ...
            if dutname == pname:
                ...
                for pin in tokens[1:]:
                    ...
                    try:
                        pinmatch = next(
                            item for item in pinlist if item["name"].upper() == upin
                        )
    ...
    if found == 0 and dutname == "":
        ...
        if dutname != pname:
            print(
                "File " + pathname + " does not have a subcircuit named " + pname + "!"
            )
            raise SyntaxError(
                "File " + pathname + " does not have a subcircuit named " + pname + "!"
            )
    ...
    for cond in lcondlist:
        ...
        if condtype == "RISETIME" or condtype == "FALLTIME":
            ...
            if "maximum" in cond:
                ...
                maxval = numeric(
                    spice_unit_convert([condunit, cond["enumerate"][-1]], "time")
                )
            elif "typical" in cond:
                maxval = numeric(
...
def substitute(
    filename, paths, tool, template, dutpath, simvals, schemline, pdkname, debug
):
                        ...
                        if cond:
                            ...
                            try:
                                ...
                                # handle this in a way that keeps ngspice happy.
                                    ...
                                    ltok = (
                                        subsline[0:watchend].replace("=", " = ").split()
                                    )
                                    if len(ltok) >= 2:
                                        ntok = ltok[:-2]
                                        ntok.append(
                                            str(numeric(ltok[-2]) + numeric(ltok[-1]))
                                        )
                                        subsline = (
                                            " ".join(ntok).replace(" = ", "=")
                                            + line[patmatch.end() :]
...
def cace_gensim(dataset, param):
    ...
    if not os.path.isfile(dutpath):
        if "verilog" not in paths:
            print(
                "No SPICE netlist exists at " + dutpath + " and no verilog path exists."
            )
    ...
    for pcond in pcondlist:
        ...
        if "unit" not in pcondlist:
            try:
                defcond = next(
                    item for item in defcondlist if item["name"] == pcond["name"]
                )
            except:
                pass
            else:
    ...
    for cond in condnames:
        if cond[-1] == "[":
            if cond in pcondnames:
                lcondlist.extend(
                    list(item for item in pcondlist if item["name"].startswith(cond))
                )
            elif cond in defcondnames:
                lcondlist.extend(
                    list(item for item in defcondlist if item["name"].startswith(cond))
                )
    ...
    for i in range(len(rlen)):
        ...
        duplist = [
            item for item in list(condition_gen(lcondlist[i])) for j in range(mpre)
        ]
