...
def run_estimate(
    subname, subpins, complist, library, rcfile, debug=False, keep=False, logname=""
):
    ...
    with open("estimate_script.tcl", "w") as ofile:
        ...
        for comp in complist:
            ...
            print(
                "    set device [info commands ${loclib}::" + devtype + "_draw]",
                file=ofile,
            )
...
def layout_estimate(inputfile, library, rcfile, debug, logfile=""):
    ...
    return run_estimate(
        topname, pindict[topname], subdict[topname], library, rcfile, debug, logfile
    )
