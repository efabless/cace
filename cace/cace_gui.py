...
class CACECharacterize(ttk.Frame):
    ...
    def init_gui(self):
        ...
        self.toppane.title_frame.title = ttk.Label(
            self.toppane.title_frame, text="User:", style="red.TLabel"
        )
        self.toppane.title_frame.user = ttk.Label(
            self.toppane.title_frame, text=username, style="blue.TLabel"
        )
        ...
        self.toppane.title2_frame.datasheet_label = ttk.Label(
            self.toppane.title2_frame, text="Datasheet:", style="normal.TLabel"
        )
        self.toppane.title2_frame.datasheet_label.grid(column=0, row=0, ipadx=5)

        # New datasheet select button
        self.toppane.title2_frame.datasheet_select = ttk.Button(
        ...
        ToolTip(
            self.toppane.title2_frame.datasheet_select, text="Select new datasheet file"
        )

        # Show path to datasheet
        self.toppane.title2_frame.path_label = ttk.Label(
            self.toppane.title2_frame, text=self.filename, style="normal.TLabel"
        )
        self.toppane.title2_frame.path_label.grid(column=2, row=0, ipadx=5, padx=10)

        # Spacer in middle moves selection button to right
        self.toppane.title2_frame.sep_label = ttk.Label(
            self.toppane.title2_frame, text=" ", style="normal.TLabel"
        )
        ...
        self.toppane.title2_frame.origin_label = ttk.Label(
            self.toppane.title2_frame, text="Netlist from:", style="normal.TLabel"
        )
        self.toppane.title2_frame.origin_label.grid(column=4, row=0, ipadx=5, padx=10)

        self.origin.set("Schematic Capture")
        self.toppane.title2_frame.origin_select = ttk.OptionMenu(
            self.toppane.title2_frame,
            self.origin,
            "Schematic Capture",
            "Schematic Capture",
        ...
        self.datasheet_viewer.create_window(
            (0, 0), window=self.datasheet_viewer.dframe, anchor="nw", tags="self.frame"
        )
    ...
    def sim_all(self):
        ...
        self.allsimbutton.configure(
            style="redtitle.TButton", text="Stop Simulations", command=self.stop_sims
        )
    ...
    def stop_sims(self):
        ...
        self.allsimbutton.configure(
            style="bluetitle.TButton", text="Simulate All", command=self.sim_all
        )
    ...
    def sim_param(self, name):
        ...
        try:
            eparam = next(
                item for item in dsheet["electrical_parameters"] if item["name"] == name
            )
        except:
            try:
                pparam = next(
    ...
    def create_datasheet_view(self):
        ...
        if "description" in dsheet:
            dframe.cframe.pdesc = ttk.Label(
                dframe.cframe, text=dsheet["description"], style="normal.TLabel"
            )
        ...
        for param in paramstodo:
            ...
            if "plot" in param:
                ...
                if "filename" in plotrec:
                    ...
                    plottext = "plot"
                dframe_plot = ttk.Label(dframe.plots, text=plottext, style=normlabel)
            ...
            if paramtype == "electrical":
                ...
                simmenu.add_command(
                    label="Edit", command=lambda param=param: self.edit_param(param)
                )
                simmenu.add_command(
                    label="Copy", command=lambda param=param: self.copy_param(param)
                )
                if "editable" in param and param["editable"] == True:
                    simmenu.add_command(
                        label="Delete",
