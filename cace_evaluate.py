...
def cace_drc(datasheet, param, toolargs=None):
    ...
    drcproc = subprocess.Popen(['magic', '-dnull', '-noconsole', '-rcfile',
		rcfile],
		stdin = subprocess.PIPE, stdout = subprocess.PIPE,
		env = newenv, start_new_session = True,
		cwd = layout_path, universal_newlines = True)

    pgroup = os.getpgid(drcproc.pid)
...
def run_and_analyze_lvs(datasheet, toolargs=None):
    ...
    if 'reports' in paths:
        ...
        if not os.path.isdir(os.path.join(root_path, reports_path)):
            os.makedirs(os.path.join(root_path, reports_path))
    ...
    if toolargs:
        ...
        if debug:
            print('cace_evaluate.py:  running netgen -batch ' + ' '.join(toolargs))
    else:
        ...
        if debug:
            print('cace_evaluate.py:  running netgen -batch lvs ', end='')
            print(layout_arg + ' ' + schem_arg + ' ', end='')
            print(lvs_setup + ' comp.out -json -blackbox')
    ...
    with subprocess.Popen(lvsargs, cwd=root_path,
		...
		env=newenv, bufsize=0) as lvsproc:
